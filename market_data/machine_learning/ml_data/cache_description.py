...
def write_description_file(
    ...
    lines.append(f"class: {resample_params.__class__.__module__}.{resample_params.__class__.__name__}")
    lines.append(f"params: {resample_params.to_str()}")
    lines.append("")
    
    # Target parameters
    lines.append("Target Parameters:")
    lines.append(f"params: {target_params_batch.get_params_dir()}")
    lines.append("")
    
    # Sequential parameters
    if seq_param is not None:
        lines.append("Sequential Parameters:")
        lines.append(f"params: {seq_param.get_params_dir()}")
        lines.append("")
...
def read_description_file(
...
) -> Optional[Tuple[ResampleParam, FeatureLabelCollection, TargetParamsBatch, Optional[SequentialFeatureParam]]]:
    """
    ...
    try:
        ...
        return resample_params, feature_collection, target_params_batch, seq_param
...
def find_cached_ml_data_with_features(
    ...
    for uuid_folder in os.listdir(base_path):
        ...
        parsed_params = read_description_file(description_path)
        if parsed_params is None:
            logger.error(f"Could not parse description file in {uuid_folder}")
            continue
        
        cached_resample, cached_features_collection, cached_target, cached_seq = parsed_params
